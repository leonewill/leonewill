<template>
  <div id="app" class="w-full min-h-screen bg-black overflow-x-hidden">
    <transition name="fade">
      <router-view />
    </transition>
  </div>
</template>

<style lang="scss">
.fade-enter-active,
.fade-leave-active {
  transition-property: opacity;
  transition-duration: 0.5s;
}
.fade-enter-active {
  transition-delay: 0s;
}
.fade-enter,
.fade-leave-active {
  opacity: 0;
}
::-moz-selection {
  color: white;
  background: rgb(208, 20, 20);
}
::selection {
  color: white;
  background: rgb(208, 20, 20);
}
.glitch-text {
  &:hover {
    color: white;
    text-transform: upercase;
    position: relative;
    display: inline-block;

    &::before,
    &::after {
      content: attr(data-text);
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      background: black;
    }
    &::before {
      left: 2px;
      text-shadow: -2px 0 #49FC00;
      clip: rect(24px, 550px, 90px, 0);
      animation: glitch-anim-2 3s infinite linear alternate-reverse;
    }
    &::after {
      left: -2px;
      text-shadow: -2px 0 #b300fc;
      clip: rect(85px, 550px, 140px, 0);
      animation: glitch-anim 2.5s infinite linear alternate-reverse;
    }
    @-webkit-keyframes glitch-anim {
      0% {
        clip: rect(34px, 9999px, 128px, 0);
      }
      4.166666666666666% {
        clip: rect(81px, 9999px, 135px, 0);
      }
      8.333333333333332% {
        clip: rect(24px, 9999px, 45px, 0);
      }
      12.5% {
        clip: rect(57px, 9999px, 130px, 0);
      }
      16.666666666666664% {
        clip: rect(71px, 9999px, 86px, 0);
      }
      20.833333333333336% {
        clip: rect(41px, 9999px, 69px, 0);
      }
      25% {
        clip: rect(47px, 9999px, 123px, 0);
      }
      29.166666666666668% {
        clip: rect(44px, 9999px, 101px, 0);
      }
      33.33333333333333% {
        clip: rect(36px, 9999px, 17px, 0);
      }
      37.5% {
        clip: rect(91px, 9999px, 54px, 0);
      }
      41.66666666666667% {
        clip: rect(98px, 9999px, 77px, 0);
      }
      45.83333333333333% {
        clip: rect(57px, 9999px, 23px, 0);
      }
      50% {
        clip: rect(75px, 9999px, 25px, 0);
      }
      54.166666666666664% {
        clip: rect(69px, 9999px, 147px, 0);
      }
      58.333333333333336% {
        clip: rect(42px, 9999px, 102px, 0);
      }
      62.5% {
        clip: rect(29px, 9999px, 63px, 0);
      }
      66.66666666666666% {
        clip: rect(122px, 9999px, 127px, 0);
      }
      70.83333333333334% {
        clip: rect(28px, 9999px, 144px, 0);
      }
      75% {
        clip: rect(121px, 9999px, 135px, 0);
      }
      79.16666666666666% {
        clip: rect(51px, 9999px, 13px, 0);
      }
      83.33333333333334% {
        clip: rect(2px, 9999px, 80px, 0);
      }
      87.5% {
        clip: rect(105px, 9999px, 16px, 0);
      }
      91.66666666666666% {
        clip: rect(57px, 9999px, 90px, 0);
      }
      95.83333333333334% {
        clip: rect(6px, 9999px, 23px, 0);
      }
      100% {
        clip: rect(51px, 9999px, 4px, 0);
      }
    }
    @keyframes glitch-anim {
      0% {
        clip: rect(34px, 9999px, 128px, 0);
      }
      4.166666666666666% {
        clip: rect(81px, 9999px, 135px, 0);
      }
      8.333333333333332% {
        clip: rect(24px, 9999px, 45px, 0);
      }
      12.5% {
        clip: rect(57px, 9999px, 130px, 0);
      }
      16.666666666666664% {
        clip: rect(71px, 9999px, 86px, 0);
      }
      20.833333333333336% {
        clip: rect(41px, 9999px, 69px, 0);
      }
      25% {
        clip: rect(47px, 9999px, 123px, 0);
      }
      29.166666666666668% {
        clip: rect(44px, 9999px, 101px, 0);
      }
      33.33333333333333% {
        clip: rect(36px, 9999px, 17px, 0);
      }
      37.5% {
        clip: rect(91px, 9999px, 54px, 0);
      }
      41.66666666666667% {
        clip: rect(98px, 9999px, 77px, 0);
      }
      45.83333333333333% {
        clip: rect(57px, 9999px, 23px, 0);
      }
      50% {
        clip: rect(75px, 9999px, 25px, 0);
      }
      54.166666666666664% {
        clip: rect(69px, 9999px, 147px, 0);
      }
      58.333333333333336% {
        clip: rect(42px, 9999px, 102px, 0);
      }
      62.5% {
        clip: rect(29px, 9999px, 63px, 0);
      }
      66.66666666666666% {
        clip: rect(122px, 9999px, 127px, 0);
      }
      70.83333333333334% {
        clip: rect(28px, 9999px, 144px, 0);
      }
      75% {
        clip: rect(121px, 9999px, 135px, 0);
      }
      79.16666666666666% {
        clip: rect(51px, 9999px, 13px, 0);
      }
      83.33333333333334% {
        clip: rect(2px, 9999px, 80px, 0);
      }
      87.5% {
        clip: rect(105px, 9999px, 16px, 0);
      }
      91.66666666666666% {
        clip: rect(57px, 9999px, 90px, 0);
      }
      95.83333333333334% {
        clip: rect(6px, 9999px, 23px, 0);
      }
      100% {
        clip: rect(51px, 9999px, 4px, 0);
      }
    }
    @-webkit-keyframes glitch-anim-2 {
      6.666666666666667% {
        clip: rect(6px, 9999px, 94px, 0);
      }
      10% {
        clip: rect(2px, 9999px, 109px, 0);
      }
      13.333333333333334% {
        clip: rect(130px, 9999px, 67px, 0);
      }
      16.666666666666664% {
        clip: rect(126px, 9999px, 124px, 0);
      }
      20% {
        clip: rect(37px, 9999px, 9px, 0);
      }
      23.333333333333332% {
        clip: rect(129px, 9999px, 33px, 0);
      }
      26.666666666666668% {
        clip: rect(105px, 9999px, 86px, 0);
      }
      30% {
        clip: rect(61px, 9999px, 37px, 0);
      }
      33.33333333333333% {
        clip: rect(131px, 9999px, 5px, 0);
      }
      36.666666666666664% {
        clip: rect(148px, 9999px, 39px, 0);
      }
      40% {
        clip: rect(66px, 9999px, 103px, 0);
      }
      43.333333333333336% {
        clip: rect(91px, 9999px, 17px, 0);
      }
      46.666666666666664% {
        clip: rect(28px, 9999px, 90px, 0);
      }
      50% {
        clip: rect(0px, 9999px, 117px, 0);
      }
      53.333333333333336% {
        clip: rect(76px, 9999px, 141px, 0);
      }
      56.666666666666664% {
        clip: rect(105px, 9999px, 122px, 0);
      }
      60% {
        clip: rect(31px, 9999px, 16px, 0);
      }
      63.33333333333333% {
        clip: rect(128px, 9999px, 41px, 0);
      }
      66.66666666666666% {
        clip: rect(48px, 9999px, 99px, 0);
      }
      70% {
        clip: rect(129px, 9999px, 34px, 0);
      }
      73.33333333333333% {
        clip: rect(36px, 9999px, 67px, 0);
      }
      76.66666666666667% {
        clip: rect(58px, 9999px, 143px, 0);
      }
      80% {
        clip: rect(103px, 9999px, 67px, 0);
      }
      83.33333333333334% {
        clip: rect(134px, 9999px, 77px, 0);
      }
      86.66666666666667% {
        clip: rect(41px, 9999px, 47px, 0);
      }
      90% {
        clip: rect(140px, 9999px, 16px, 0);
      }
      93.33333333333333% {
        clip: rect(7px, 9999px, 96px, 0);
      }
      96.66666666666667% {
        clip: rect(54px, 9999px, 71px, 0);
      }
      100% {
        clip: rect(1px, 9999px, 73px, 0);
      }
    }
    @keyframes glitch-anim-2 {
      6.666666666666667% {
        clip: rect(6px, 9999px, 94px, 0);
      }
      10% {
        clip: rect(2px, 9999px, 109px, 0);
      }
      13.333333333333334% {
        clip: rect(130px, 9999px, 67px, 0);
      }
      16.666666666666664% {
        clip: rect(126px, 9999px, 124px, 0);
      }
      20% {
        clip: rect(37px, 9999px, 9px, 0);
      }
      23.333333333333332% {
        clip: rect(129px, 9999px, 33px, 0);
      }
      26.666666666666668% {
        clip: rect(105px, 9999px, 86px, 0);
      }
      30% {
        clip: rect(61px, 9999px, 37px, 0);
      }
      33.33333333333333% {
        clip: rect(131px, 9999px, 5px, 0);
      }
      36.666666666666664% {
        clip: rect(148px, 9999px, 39px, 0);
      }
      40% {
        clip: rect(66px, 9999px, 103px, 0);
      }
      43.333333333333336% {
        clip: rect(91px, 9999px, 17px, 0);
      }
      46.666666666666664% {
        clip: rect(28px, 9999px, 90px, 0);
      }
      50% {
        clip: rect(0px, 9999px, 117px, 0);
      }
      53.333333333333336% {
        clip: rect(76px, 9999px, 141px, 0);
      }
      56.666666666666664% {
        clip: rect(105px, 9999px, 122px, 0);
      }
      60% {
        clip: rect(31px, 9999px, 16px, 0);
      }
      63.33333333333333% {
        clip: rect(128px, 9999px, 41px, 0);
      }
      66.66666666666666% {
        clip: rect(48px, 9999px, 99px, 0);
      }
      70% {
        clip: rect(129px, 9999px, 34px, 0);
      }
      73.33333333333333% {
        clip: rect(36px, 9999px, 67px, 0);
      }
      76.66666666666667% {
        clip: rect(58px, 9999px, 143px, 0);
      }
      80% {
        clip: rect(103px, 9999px, 67px, 0);
      }
      83.33333333333334% {
        clip: rect(134px, 9999px, 77px, 0);
      }
      86.66666666666667% {
        clip: rect(41px, 9999px, 47px, 0);
      }
      90% {
        clip: rect(140px, 9999px, 16px, 0);
      }
      93.33333333333333% {
        clip: rect(7px, 9999px, 96px, 0);
      }
      96.66666666666667% {
        clip: rect(54px, 9999px, 71px, 0);
      }
      100% {
        clip: rect(1px, 9999px, 73px, 0);
      }
    }
  }
}
</style>
